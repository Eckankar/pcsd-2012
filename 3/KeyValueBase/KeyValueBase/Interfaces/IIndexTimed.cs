...
{
  public interface IIndexTimed<K, V> 
    where K : IKey<K>
    where V : IValue
  {
    /// <summary>
    ...
    /// <exception cref="KeyNotFoundException">If the key is not present</exception>
    /// <exception cref="IOException">If an I/O error occurs</exception>
    void Update(K key, TimestampLog newLsn, V newValue);
    ...
    KeyValuePair<TimestampLog, V> Get(K key);
    ...
    KeyValuePair<TimestampLog,IEnumerable<V>> Scan(K begin, K end);
    ...
    /// <exception cref="IOException">If an I/O error occurs</exception>
    /// <returns>list of values</returns>
    KeyValuePair<TimestampLog, IEnumerable<V>> AtomicScan(K begin, K end);
    ...
    void BulkPut(IEnumerable<KeyValuePair<K, V>> pairs, TimestampLog lsn);
  }
}
